...
router.get('/list', checkLogin, function (req, res, next) {
    ...
    s.updateUserId, d.name as departName, m.name as majorName, c.name as className, 
...
router.get('/edit/:id', checkLogin, function (req, res, next) {
    ...
    `, [id], function (err, result) {
            ...
            if (result[0].length != 1) {
                res.json({ code: 201, message: "所编辑学生不存在" });
                return;
            }
...
router.post('/edit', checkLogin, function (req, res, next) {
    ...
    pool.query("select * from students where id = ? ", [id], function (err, result) {
        ...
        if (result[0].length > 1 || result[0].length < 1) {
            res.json({ code: 201, message: "你编辑的学生不存在" });
            return;
        }

        var sql = `update students set sno=?,name=?,sex=?,birthday=?,card=?,
        majorId=?,classId=?,departId=?,nativePlace=?,address=?,qq=?,phone=?,email=?,updateTime=?,updateUserId=? where
        id = ?`;
...
router.post('/remove', checkLogin, function (req, res, next) {
    ...
    pool.query(`UPDATE students SET status=1 WHERE id=?`, [id], function (err, result1) {
        if (err) {
            res.json({ code: 201, message: "数据库操作异常！" });
            return;
        }
